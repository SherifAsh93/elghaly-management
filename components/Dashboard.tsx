...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        console.warn("Dashboard fetch failed, using defaults:", err);
  ...
  if (loading) return (
    <div className="p-20 flex flex-col items-center justify-center">
      ...
      <p className="font-black text-slate-400 animate-pulse">جاري تحميل لوحة التحكم...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <StatCard title="ربطات المخزن" value={(stats.totalInventory || 0).toLocaleString()} icon="🪵" color="bg-indigo-600" />
        <StatCard title="إجمالي المبيعات" value={(stats.totalSales || 0).toLocaleString() + ' ج.م'} icon="💹" color="bg-emerald-600" />
        <StatCard title="الدخل المتوقع" value={(stats.expectedIncome || 0).toLocaleString() + ' ج.م'} icon="💰" color="bg-amber-500" />
        <StatCard title="عدد العملاء" value={(stats.clientCount || 0).toString()} icon="🤝" color="bg-violet-600" />
        <StatCard title="سلف الموظفين" value={(stats.pendingAdvances || 0).toLocaleString() + ' ج.م'} icon="💸" color="bg-rose-600" />
      </div>
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-10 pb-4 border-b border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <span className="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em] mb-2 block">التحليل المالي المرئي</span>
            <h3 className="text-3xl font-black text-slate-800 tracking-tight">إحصائيات الدخل والنمو</h3>
            <p className="text-sm text-slate-400 font-bold mt-1">تتبع التدفقات المالية لآخر 6 أشهر من العمليات</p>
          </div>
          <div className="flex gap-2">
             <div className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                <div className="w-2 h-2 bg-orange-50 rounded-full"></div>
                <span className="text-[10px] font-black text-slate-600">المبيعات الشهرية</span>
             </div>
          </div>
        </div>
          ...
          <div className="flex items-end justify-between h-72 gap-6">
            {stats.monthlyIncome && stats.monthlyIncome.length > 0 ? (
                ...
                <div key={i} className="flex-1 flex flex-col items-center gap-6 group">
                  <div className="relative w-full flex flex-col justify-end h-56">
                    ...
                    <div 
                      ...
                      style={{ height: `${maxMonthly > 0 ? (m.income / maxMonthly) * 100 : 0}%`, minHeight: '8px' }}
                    >
                      <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-t-[1.5rem]"></div>
                    </div>
                  </div>
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-white px-3 py-1 rounded-full border border-slate-100 shadow-sm">{m.month}</div>
                </div>
              ))
            ) : (
              <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 font-bold uppercase tracking-widest gap-4">
                ...
                {errorOccurred && <p className="text-[10px] text-rose-400 font-black">خطأ في الاتصال بالسيرفر - يرجى تحديث الصفحة</p>}
              </div>
            )}
          </div>
        </div>
...
const StatCard: React.FC<{ title: string; value: string; icon: string; color: string }> = ({ title, value, icon, color }) => (
  <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden relative text-center">
    <div className={`${color} w-14 h-14 mx-auto rounded-2xl flex items-center justify-center text-2xl text-white shadow-lg group-hover:scale-110 transition-transform mb-4`}>
      {icon}
    </div>
