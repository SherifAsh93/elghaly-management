...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { name, code, type, units, sheetsPerUnit, buyPricePerSheet, sellPricePerSheet, thickness, height, width } = body;
    ...
    await query(
      ...
      WHERE id = ?`,
      [name, code, type, safeUnits, safeSheets, parseFloat(buyPricePerSheet), parseFloat(sellPricePerSheet), parseFloat(thickness), parseFloat(height), parseFloat(width), safeUnits, totalSheets, id]
    );
