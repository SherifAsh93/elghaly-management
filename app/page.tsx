...
export default function Home() {
  ...
  if (!userRole) {
    return (
      <div
        className="min-h-screen bg-slate-900 flex items-center justify-center p-4 font-sans"
        dir="rtl"
      >
        <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/10">
          <div className="bg-[#1e293b] p-12 text-center text-white relative">
            ...
            <p className="text-orange-400 text-[10px] mt-2 font-bold tracking-[0.3em] uppercase opacity-80">
              لاستيراد وتجارة الأخشاب
            </p>
          </div>
          ...
          <div className="p-10 space-y-8 bg-white">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              {error && (
                <p className="text-rose-500 text-[10px] font-black text-center animate-bounce">
                  {error}
                </p>
              )}
              <button
                type="submit"
                className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg shadow-xl hover:bg-orange-600 transition-all active:scale-95"
              >
                دخول المدير
              </button>
            </form>
            ...
            <button
              ...
              className="w-full bg-slate-50 text-slate-600 py-5 rounded-2xl font-black border border-slate-100 hover:bg-slate-100 transition-all active:scale-95"
            >
              دخول سريع للمبيعات
            </button>
          </div>
  ...
  return (
    <div
      className="flex h-screen bg-[#0f172a] text-right font-sans overflow-hidden"
      dir="rtl"
    >
      <Sidebar
        activeTab={activeTab}
        ...
        <header className="flex items-center justify-between bg-white px-8 py-6 shadow-sm border-b border-gray-50 z-30 lg:rounded-tr-[3rem]">
          <div className="flex items-center gap-6">
            <button
              ...
              className="lg:hidden p-3 bg-slate-100 rounded-2xl text-slate-600 hover:bg-orange-50 hover:text-orange-600 transition-colors"
            >
              <svg
          ...
          <div className="flex items-center gap-6">
            <div className="hidden xl:flex flex-col items-end">
              <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full uppercase tracking-widest mb-1">
                اتصال سحابي آمن
              </span>
